  ...
  validate() {
    ...
    const processConfig = _config => {
      ...
      if (_.has(this.serverless, 'service.package') && this.serverless.service.package.individually) {
        ...
        const allEntryFunctions = this.serverless.service
          .getAllFunctions()
          .map(funcName => this.serverless.service.getFunction(funcName))
          .filter(
            func =>
              (func.runtime === undefined &&
                this.serverless.providers.aws.runtime !== undefined &&
                this.serverless.providers.aws.runtime.startsWith('node')) ||
              (func.runtime !== undefined && func.runtime.startsWith('node'))
          )
          .map(func => {
